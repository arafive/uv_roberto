...
for delay in np.arange(0, 90, 1):
    for m in [0, 30]:
        ...
        gg3_ts = adesso_timestamp - pd.Timedelta(hours=72)
        gg4_ts = adesso_timestamp - pd.Timedelta(hours=96)
        
        adesso = adesso_timestamp.strftime('%Y%m%d%H%M')
        gg4 = gg4_ts.strftime('%Y%m%d%H%M')
        ...
        WHERE
            ...
            DATA.DTRF > TO_DATE('{gg4}', 'YYYYMMDDHH24MI') AND
